...
int gen_white_knight_moves(state_t *gs, move_t *move_list, int start) {
  ...
  while (knight) {
    ...
    while (attacks) {
      POP_LSB(attacks, to);
      move_list[count++] =
          MOVE(from, to, ((AS_BIT(to) & gs->bp) ? CAPTURE : NORMAL_MOVE));
    }
  }
  return count - start;
}
...
int gen_black_knight_moves(state_t *gs, move_t *move_list, int start) {
  ...
  while (knight) {
    ...
    while (attacks) {
      POP_LSB(attacks, to);
      move_list[count++] =
          MOVE(from, to, ((AS_BIT(to) & gs->wp) ? CAPTURE : NORMAL_MOVE));
    }
  }
  return count - start;
}
...
const bitboard_t ROOK_MAGIC[64] = {
    ...
    0x14800401802800ULL,   0x2080040080800200ULL, 0x124080204001000ULL,
